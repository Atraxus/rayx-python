...
#undef X
        ...
        .def("trace",
             [](rayx::Beamline& bl, bool sequential, std::optional<uint32_t> seed, std::optional<int> max_events,
                std::optional<int> device_index, rayx::DeviceConfig::DeviceType device_type) {
                 ...
                 // Select the compute device. By default (device_index = None) the best available device of the
                 // requested type is chosen automatically; otherwise the device with the given index is used. The
                 // index refers to the list returned by list_devices(device_type). We validate the index here instead
                 // of letting enableDeviceByIndex() call RAYX_EXIT, which would terminate the Python interpreter.
    ...
    m.def(
        ...
        [](rayx::DeviceConfig::DeviceType device_type) {
            ...
            // Each entry maps the index accepted by trace(device_index=...) to a human-readable name and its type.
